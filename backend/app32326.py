...
class App(QWidget):
    ...
    def _build_status_tab(self):
        ...
        btn_reconnect_mobile = QPushButton("Reconnect Mobile Data")
        btn_reconnect_mobile.clicked.connect(self.reconnect_mobile_data)
        row.addWidget(btn_reconnect_mobile)
    ...
    def _render_status(self, data: dict):
        ...
        latest = data.get("latest_network_log", {}) or {}
        pwr = data.get("power", {}) or {}
        md = data.get("mobile_data", {}) or {}

        md_connected = "YES" if md.get("connected") else "NO"
        md_session = md.get("session_status") or "unknown"
        md_iface = md.get("iface") or "wwan0"
        md_iface_up = "YES" if md.get("wwan0_up") else "NO"
        md_iface_ipv4 = md.get("wwan0_ipv4") or "-"
        md_ipv4 = md.get("ipv4") or "-"
        md_gateway = md.get("gateway") or "-"
        md_dns1 = md.get("dns1") or "-"
        md_dns2 = md.get("dns2") or "-"
        md_radio = md.get("radio_interface") or "-"
        md_cap = md.get("data_capability") or "-"
        md_operator = md.get("operator") or "-"
        md_rssi = md.get("rssi_dbm")
        md_rssi_text = f"{md_rssi} dBm" if md_rssi is not None else "-"
        md_notes = md.get("notes") or []
        md_notes_text = "\n".join(f"- {x}" for x in md_notes) if md_notes else "-"

        txt = f"""DEVICE HEARTBEAT
-----------------------
Status: {hb.get('status', 'UNKNOWN')}
Last seen: {hb.get('last_seen_ts')}
Last online: {hb.get('last_online_ts')} ({hb.get('last_online_ago', 'unknown')})
Last offline: {hb.get('last_offline_ts')}

MOBILE DATA (USB/QMI)
-----------------------
Connected: {md_connected}
Session status: {md_session}
Interface: {md_iface}
Interface up: {md_iface_up}
Interface IPv4: {md_iface_ipv4}
QMI IPv4: {md_ipv4}
Gateway: {md_gateway}
DNS1: {md_dns1}
DNS2: {md_dns2}
Operator: {md_operator}
Radio interface: {md_radio}
Data capability: {md_cap}
RSSI: {md_rssi_text}
Notes:
{md_notes_text}
...
POWER STATUS
-----------------------
Undervoltage now: {yn(pwr.get('undervoltage_now'))}
Undervoltage since boot: {yn(pwr.get('undervoltage_boot'))}
Throttling now: {yn(pwr.get('throttling_now'))}
Throttling since boot: {yn(pwr.get('throttling_boot'))}
Throttled raw: {pwr.get('raw', 'N/A')}

MODEM STATUS SUMMARY
-----------------------
AT meaning: {data.get('AT_MEANING')}
CSQ meaning: {data.get('CSQ_MEANING')}
CREG meaning: {data.get('CREG_MEANING')}
...
"""
    ...
    def reconnect_mobile_data(self):
        reply = QMessageBox.question(
            self,
            "Reconnect Mobile Data",
            "Run mobile-data reconnect now on the server/Pi?"
        )
        if reply != QMessageBox.Yes:
            return

        try:
            resp = http_post("/mobile_data/reconnect", {}, timeout=40)
            md = (resp or {}).get("mobile_data", {}) or {}
            msg = (resp or {}).get("message", "Reconnect finished.")
            connected = "YES" if md.get("connected") else "NO"
            iface_ipv4 = md.get("wwan0_ipv4") or md.get("ipv4") or "-"
            session = md.get("session_status") or "unknown"
    ...
    def refresh_logs(self):
        data = http_get("/logs/ongoing")
        current_sb = self.logs_box.verticalScrollBar()
        current_pos = current_sb.value()
        at_bottom = current_pos >= max(0, current_sb.maximum() - 4)

        activity = data.get("activity", []) or []
        gps = data.get("gps", []) or []
        sms = data.get("sms_sent", []) or []
        sms_inbox = data.get("sms_inbox", []) or []
        cameras = data.get("cameras", []) or []
        md = data.get("mobile_data", {}) or {}
        present_devices = data.get("present_devices", []) or []
        ssh_sessions = data.get("ssh_sessions", []) or []
        ssh_summary = data.get("ssh_access_summary", []) or []
        ssh_log_tail = data.get("ssh_log_tail", []) or []
        kernel_log_tail = data.get("kernel_module_log_tail", []) or []

        lines = ["=== MOBILE DATA STATUS ==="]
        lines.append(f"Connected: {'YES' if md.get('connected') else 'NO'}")
        lines.append(f"Session Status: {md.get('session_status')}")
        lines.append(f"Interface: {md.get('iface')}")
        lines.append(f"Interface IPv4: {md.get('wwan0_ipv4')}")
        lines.append(f"QMI IPv4: {md.get('ipv4')}")
        lines.append(f"Gateway: {md.get('gateway')}")
        lines.append(f"DNS1: {md.get('dns1')}")
        lines.append(f"DNS2: {md.get('dns2')}")
        lines.append(f"Operator: {md.get('operator')}")
        lines.append(f"Radio Interface: {md.get('radio_interface')}")
        lines.append(f"Data Capability: {md.get('data_capability')}")
        lines.append(f"RSSI: {md.get('rssi_dbm')}")

        lines.append("\n=== PRESENT CONNECTED DEVICES ===")
        if present_devices:
            lines.extend([str(x) for x in present_devices])
        else:
            lines.append("No present-device info available")

        lines.append("\n=== GPS LOGS (last 20) ===")
        for g in gps:
            lines.append(f"{g.get('ts')} | UTC={g.get('utc')} | LAT={g.get('lat')} | LON={g.get('lon')} | SPEED={g.get('speed')} | SAT={g.get('sat')}")

        lines.append("\n=== SMS SENT LOG (last 20) ===")
        for s_row in sms:
            lines.append(f"{s_row.get('ts')} | TO {s_row.get('number')} | ok={s_row.get('ok')} | {s_row.get('message')}")

        lines.append("\n=== SMS INBOX (last 20) ===")
        for s_row in sms_inbox:
            lines.append(f"{s_row.get('ts')} | FROM {s_row.get('from_number')} | {s_row.get('message')}")
        ...
        if cameras:
            for c in cameras:
                ...
                if present is None:
                    present = c.get("exists")
                present_text = "YES" if present else "NO"
                live_text = "YES" if c.get("streaming") else "NO"
                size_text = "-"
                if c.get("width") and c.get("height"):
                    size_text = f"{c.get('width')}x{c.get('height')}"
                fps = c.get("fps")
                fps_text = f"{fps:.1f}" if isinstance(fps, (int, float)) else "-"
                last_frame = c.get("last_frame_ts") or "-"
                last_error = c.get("last_error") or "-"
                name = c.get("name") or "-"
                status = c.get("status") or "-"
                lines.append(
                    f"{c.get('device')} | present={present_text} | status={status} | live={live_text} | "
                    f"size={size_text} | fps={fps_text} | name={name} | last_frame={last_frame} | last_error={last_error}"
                )
        else:
        ...
        lines.append("\n=== CURRENT SSH USERS ===")
        if ssh_sessions:
            for s_row in ssh_sessions:
                user = s_row.get("user") or "-"
                addr = s_row.get("from") or "local"
                tty = s_row.get("tty") or "-"
                date = s_row.get("date") or "-"
                lines.append(f"{user} | {addr} | {tty} | {date}")
        else:
            lines.append("No active SSH sessions")

        lines.append("\n=== SSH ACCESS (SIMPLE) ===")
        if ssh_summary:
            lines.extend([str(x) for x in ssh_summary])
        else:
            lines.append("No SSH access summary available")

        lines.append("\n=== SSH LOG TAIL ===")
        if ssh_log_tail:
            lines.extend(ssh_log_tail)
        else:
            lines.append("No SSH logs available")

        lines.append("\n=== KERNEL / MODULE LOG TAIL ===")
        if kernel_log_tail:
            lines.extend(kernel_log_tail)
        else:
            lines.append("No kernel/module logs available")
        ...
        new_sb = self.logs_box.verticalScrollBar()
        if at_bottom:
            new_sb.setValue(new_sb.maximum())
        else:
            new_sb.setValue(min(current_pos, new_sb.maximum()))

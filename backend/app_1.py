...
def detect_base(timeout: int = 2) -> str:
    ...
    raise RuntimeError(
        ...
        "- PC & Pi connected to the same network or ZeroTier\n"
        "- Correct Pi IP is set in app.py\n"
    )
...
class App(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SIM7000C Control Panel + GPS + Logs + Export")
    ...
    def _build_status_tab(self):
        ...
        btn_poweroff = QPushButton("Power Off Pi")
...
"""
    ...
    def _build_groups_tab(self):
        ...
        btn_reload = QPushButton("Reload Group (from Pi)")
    ...
    def refresh_logs(self):
        ...
        for s in sms:
            lines.append(f"{s.get('ts')} | {s.get('number')} | ok={s.get('ok')} | {s.get('message')}")
    ...
    def render_gps(self, data: dict):
        lines = []
        for k in ["run", "fix", "utc", "lat", "lon", "alt", "speed", "course", "sat", "location", "google_maps"]:
            if k in data:
                lines.append(f"{k}: {data.get(k)}")
        lines.append("")
        lines.append("raw:")
        lines.append(str(data.get("raw", "")))
